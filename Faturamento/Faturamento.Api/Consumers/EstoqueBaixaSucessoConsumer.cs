...
public class EstoqueBaixaSucessoConsumer : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IConfiguration _configuration;
    private readonly ILogger<EstoqueBaixaSucessoConsumer> _logger;
    private IConnection? _connection;
    private IModel? _channel;

    private const string ExchangeName = "korp.eventos";
    private const string QueueName = "faturamento.estoque.baixa.sucesso";
    private const string RoutingKey = "estoque.baixa.sucesso";

    public EstoqueBaixaSucessoConsumer(
        IServiceProvider serviceProvider,
        IConfiguration configuration,
        ILogger<EstoqueBaixaSucessoConsumer> logger)
    {
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (_, ea) =>
        {
            ...
            try
            {
                var evento = JsonSerializer.Deserialize<EstoqueBaixaSucessoEvent>(json);

                _logger.LogInformation("Mensagem de sucesso de estoque recebida.");
                    ...
                    nota.Status == NotaFiscalStatus.ProcessandoFechamento)
                {
                    _logger.LogInformation(
                        "Fechando nota {NotaFiscalId} por mensagem de sucesso. CorrelationId: {CorrelationId}",
                        evento.NotaFiscalId,
